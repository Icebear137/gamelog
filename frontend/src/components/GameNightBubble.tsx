...
import { CalendarDays, Monitor, Check, HelpCircle, X, Gamepad2 } from "lucide-react";
...
const RSVP_OPTIONS = [
  { value: "going" as const, label: "Going", icon: Check, active: "bg-emerald-500/20 text-emerald-300 border-emerald-500/40", passive: "bg-white/5 text-gray-400 hover:bg-white/10 border-white/10" },
  { value: "maybe" as const, label: "Maybe", icon: HelpCircle, active: "bg-amber-500/20 text-amber-300 border-amber-500/40", passive: "bg-white/5 text-gray-400 hover:bg-white/10 border-white/10" },
  { value: "no" as const, label: "No", icon: X, active: "bg-red-500/20 text-red-300 border-red-500/40", passive: "bg-white/5 text-gray-400 hover:bg-white/10 border-white/10" },
];

export default function GameNightBubble({ gameNight, conversationId, currentUserId, isOwn }: Props) {
    ...
    onSuccess: (res) => {
      ...
      qc.setQueryData(
        ...
        (old: { messages: ChatMessage[] } | undefined) => {
          if (!old) return old;
          return { ...old, messages: old.messages.map((m) => m.gameNight?.id === gameNight.id ? { ...m, gameNight: updated } : m) };
        }
      );
    },
    onError: (err: any) => {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className={`px-3 pt-2.5 pb-2 border-b ${isOwn ? "border-emerald-500/20" : "border-white/8"}`}>
        <Flex align="center" gap="1" className="text-[10px] font-semibold uppercase tracking-wide text-emerald-400 mb-1">
          <CalendarDays size={11} />
          Game Night{isPast ? " · Past" : ""}
        </Flex>
        <Text as="p" size="2" weight="medium" className="font-semibold leading-snug">{gameNight.title}</Text>
        ...
        >
          {gameNight.game.coverImage ? (
            <div className="relative w-8 h-10 rounded overflow-hidden shrink-0">
              <Image src={gameNight.game.coverImage} alt={gameNight.game.name} fill className="object-cover" sizes="32px" />
            </div>
          ) : (
          ...
          <span className="text-xs text-gray-300 truncate">{gameNight.game.name}</span>
        </Link>
      )}

      {/* Note */}
      {gameNight.note && (
        <div className={`px-3 py-1.5 border-b ${isOwn ? "border-emerald-500/20" : "border-white/8"}`}>
          <p className="text-[11px] text-gray-500 leading-snug">{gameNight.note}</p>
        </div>
      )}
                ...
                {going.slice(0, 6).map((r, i) => (
                  ...
                  >
                    {r.user.avatar ? (
                      ...
                      <span className="text-white font-bold uppercase leading-none select-none" style={{ fontSize: 5 }}>{r.user.username[0]}</span>
                    )}
                  </div>
                ))}
              </div>
