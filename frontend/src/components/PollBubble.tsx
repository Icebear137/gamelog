...
import { BarChart2, CheckCircle2, Clock, LockKeyhole, EyeOff, ChevronDown, ChevronUp } from "lucide-react";
...
function VoterList({ option, anonymous }: { option: PollOptionData; anonymous: boolean }) {
    ...
    <div className="mt-1">
      ...
      >
        <div className="flex -space-x-1">
          {preview.map((v, i) => (
            <div key={i} className="w-4 h-4 rounded-full border border-zinc-900 overflow-hidden shrink-0">
              <Avatar src={v.user?.avatar} username={v.user?.username ?? "?"} size="xs" />
            </div>
          ))}
        </div>
        <span>{count} vote{count !== 1 ? "s" : ""}</span>
...
export default function PollBubble({ poll, conversationId, currentUserId, isOwn }: Props) {
  ...
  const isExpired = !!poll.endsAt && (remaining !== null ? remaining <= 0 : new Date(poll.endsAt) <= new Date());
  ...
  function updateCache(updatedPoll: PollData) {
    qc.setQueryData(["messages", conversationId], (old: { messages: ChatMessage[] } | undefined) => {
      if (!old) return old;
      return {
        ...old,
  ...
  const voteMutation = useMutation({
    mutationFn: (optionId: string) => api.post(`/api/messages/polls/${poll.id}/vote`, { optionId }),
    onSuccess: (res) => updateCache(res.data.poll),
    onError: (err: any) => dispatchToast(err?.response?.data?.error ?? "Failed to vote", "error"),
  });

  const closeMutation = useMutation({
    mutationFn: () => api.post(`/api/messages/polls/${poll.id}/close`),
    onSuccess: (res) => updateCache(res.data.poll),
    onError: (err: any) => dispatchToast(err?.response?.data?.error ?? "Failed to close poll", "error"),
  });

  return (
    <div className={`rounded-2xl overflow-hidden border max-w-64 ${
      isOwn ? "border-violet-500/30 bg-violet-900/20 rounded-br-sm" : "border-white/10 bg-white/5 rounded-bl-sm"
    }`}>
      {/* Header */}
      <div className={`px-3 pt-2.5 pb-1.5 border-b ${isOwn ? "border-violet-500/20" : "border-white/8"}`}>
        <Flex align="center" justify="between" className="mb-1">
          <Flex align="center" gap="1" className="text-[10px] font-semibold uppercase tracking-wide text-violet-400">
            <BarChart2 size={11} />
            Poll
          ...
          <div className="flex items-center gap-1.5">
            ...
            ) : poll.endsAt && remaining !== null ? (
              <span className={`flex items-center gap-0.5 text-[10px] ${remaining < 60_000 ? "text-red-400" : "text-gray-500"}`}>
                <Clock size={9} /> {fmtRemaining(remaining)}
              </span>
            ) : null}
          </div>
        </Flex>
        <p className="text-sm font-semibold leading-snug text-white">{poll.question}</p>
      </div>
              ...
              >
                <div
                  className={`absolute inset-0 rounded-lg transition-all duration-500 ${
                    isMyVote ? "bg-violet-500/30" : isClosed ? "bg-white/4" : "bg-white/6 group-hover/opt:bg-white/10"
                  }`}
                  style={{ width: (hasVoted || isClosed) ? `${Math.max(pct, 4)}%` : "100%" }}
                />
